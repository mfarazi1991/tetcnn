...
for cv in range(1):
    with open(os.path.join('sbatch_jobs', 'job_' + exp_name + '_' + str(cv) + '.sh'), 'w') as fout:
        ...
        out_lines.append('#SBATCH -o /home/mfarazi/tetCNN/scripts/job_logs/' + str(pos_name) + '_' + str(neg_name) + '_' + exp_name + '_' + str(cv) + '.out')
        out_lines.append('#SBATCH --mail-type=END             # Send a notification when the job starts, stops, or fails')
        out_lines.append('#SBATCH --mail-user=mfarazi@asu.edu # send-to address\n')
        ...
        out_lines.append('python /home/mfarazi/tetCNN/src/tetCNN.py --pos ' + pos_folder + ' --neg ' + neg_folder + ' --n_epoch ' + str(n_epoch) + ' --lr ' + str(lr) + ' --wd ' + str(wd) + ' --cv ' + str(cv) + ' --load')
